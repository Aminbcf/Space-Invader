...
bool test_controller_commands(void) {
    ...
    controller_execute_command(controller, CMD_START_GAME);
    TEST_ASSERT_EQ(model.state, STATE_PLAYING);
    
    // Test movement commands
    controller_execute_command(controller, CMD_MOVE_LEFT);
    TEST_ASSERT_LT(model.player.hitbox.x, (GAME_AREA_WIDTH / 2) - (PLAYER_WIDTH / 2));
    
    int current_x = model.player.hitbox.x;
    controller_execute_command(controller, CMD_MOVE_RIGHT);
    TEST_ASSERT_GT(model.player.hitbox.x, current_x);
