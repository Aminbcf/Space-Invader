...
#define LOAD_TEXTURE_SAFE(path, dest)                                          \
  ...
  LOAD_TEXTURE_SAFE("pictures/pwr_triple.bmp", view->pwr_tex[1]); // PWR_TRIPLE_SHOT
  LOAD_TEXTURE_SAFE("pictures/pwr_strong.bmp", view->pwr_tex[2]); // PWR_STRONG_MISSILE
  LOAD_TEXTURE_SAFE("pictures/pwr_shield.bmp", view->pwr_tex[3]); // PWR_SHIELD (actually shield is index 3 in enum)
...
bool sdl_view_init(SDLView *view, int width, int height) {
  ...
  SDL_SetRenderLogicalPresentation(view->renderer, GAME_AREA_WIDTH + 200, SCREEN_HEIGHT, 
...
void sdl_view_draw_hud(SDLView *view, const GameModel *model) {
  ...
  for (int i = 0; i < model->players[0].lives && i < 5; i++) {
    ...
    if (view->player_tex[0][0]) SDL_RenderTexture(view->renderer, view->player_tex[0][0], NULL, &life);
  }
  ...
  if (model->two_player_mode) {
    ...
    for (int i = 0; i < model->players[1].lives && i < 5; i++) {
      ...
      if (view->player_tex[1][0]) SDL_RenderTexture(view->renderer, view->player_tex[1][0], NULL, &life);
      else if (view->player_tex[0][0]) SDL_RenderTexture(view->renderer, view->player_tex[0][0], NULL, &life);
    }
  }
  ...
  for (int p = 0; p < 2; p++) {
    if (model->players[p].is_active && model->players[p].active_powerup != PWR_NONE) {
      ...
      SDL_FRect t_fill = {650.0f, (float)py + 5, model->players[p].powerup_timer * 20.0f, 10.0f};
...
static void sdl_view_render_difficulty_menu(SDLView *view,
  ...
  const char *items[] = {"EASY   - 3 Levels Only",
                         "NORMAL - 4 Levels with Boss",
                         "HARD   - Faster & Harder",
                         "ROGUE  - Infinite & Randomized"};
  int y_start = 230;
  int y_spacing = 60;
...
static const char* get_key_name(int keycode) {
    static char buf[32];
    switch(keycode) {
        case 1073741904: return "LEFT";
        case 1073741903: return "RIGHT";
        case 1073741906: return "UP";
        case 1073741905: return "DOWN";
        case 32: return "SPACE";
        case 1073742049: return "L-SHIFT";
        case 1073742048: return "L-CTRL";
...
static void sdl_view_render_controls_menu(SDLView *view, const GameModel *model) {
  ...
  draw_text_centered(view, "CONTROLS", 60, (SDL_Color){COLOR_TEXT_HIGHLIGHT}, true);
  
  if (model->waiting_for_key) {
      draw_text_centered(view, "PRESS A KEY...", 100, (SDL_Color){255, 255, 0, 255}, false);
  }
  ...
  for (int i = 0; i < 5; i++) {
      char buf[64];
      snprintf(buf, 64, "%s: %s", actions[i], get_key_name(model->keybinds_p1[i]));
      SDL_Color c = (model->menu_selection == i) ? sel : wht;
      if (model->waiting_for_key && model->editing_keybind == i) c = (SDL_Color){0, 255, 100, 255};
  ...
  for (int i = 0; i < 5; i++) {
      char buf[64];
      snprintf(buf, 64, "%s: %s", actions[i], get_key_name(model->keybinds_p2[i]));
      SDL_Color c = (model->menu_selection == 5 + i) ? sel : wht;
      if (model->waiting_for_key && model->editing_keybind == 5 + i) c = (SDL_Color){0, 255, 100, 255};
...
void sdl_view_render_game_scene(SDLView *view, const GameModel *model) {
  ...
  for (int pIdx = 0; pIdx < 2; pIdx++) {
    if (!model->players[pIdx].is_active) continue;
    
    float p_scale = 2.0f;
    float p_w = (float)model->players[pIdx].hitbox.width * p_scale;
    float p_h = (float)model->players[pIdx].hitbox.height * p_scale;
    float p_x = (float)model->players[pIdx].hitbox.x - (p_w - (float)model->players[pIdx].hitbox.width) / 2.0f;
    float p_y = (float)model->players[pIdx].hitbox.y - (p_h - (float)model->players[pIdx].hitbox.height) / 2.0f;
    // Shield effect (Bubble)
    if (model->players[pIdx].active_powerup == PWR_SHIELD) {
        SDL_SetRenderDrawColor(view->renderer, 0, 200, 255, 100);
        float cx = (float)model->players[pIdx].hitbox.x + (float)model->players[pIdx].hitbox.width / 2;
        float cy = (float)model->players[pIdx].hitbox.y + (float)model->players[pIdx].hitbox.height / 2;
    ...
    if (model->players[pIdx].active_powerup == PWR_TRIPLE_SHOT) {
        draw_particle_effect(view, (float)model->players[pIdx].hitbox.x + (float)model->players[pIdx].hitbox.width/2,
                            (float)model->players[pIdx].hitbox.y + (float)model->players[pIdx].hitbox.height/2,
                            40.0f, 0x00FFFF);
    }

    // Power-up collection particles (if just collected, but we don't have a timer for that here, so just subtle glow)
    if (model->players[pIdx].active_powerup != PWR_NONE) {
        draw_particle_effect(view, (float)model->players[pIdx].hitbox.x + (float)model->players[pIdx].hitbox.width/2,
                            (float)model->players[pIdx].hitbox.y + (float)model->players[pIdx].hitbox.height,
                            20.0f, 0xFFFFFF);
    }
    ...
    if (view->player_tex[pIdx][p_anim_idx]) {
        SDL_RenderTexture(view->renderer, view->player_tex[pIdx][p_anim_idx], NULL, &p_dst);
    } else if (view->player_tex[pIdx][0]) {
        SDL_RenderTexture(view->renderer, view->player_tex[pIdx][0], NULL, &p_dst);
    } else {
    ...
    if (model->players[pIdx].combo_count >= 5) {
        ...
        draw_text(view, combo_buf, (int)p_x, (int)p_y - 15, (SDL_Color){255, 200, 0, 200});
    }
  }
  ...
  for (int i = 0; i < 10; i++) {
    if (model->powerups[i].alive) {
      ...
      float pw_x = model->powerups[i].hitbox.x - (pw_w - model->powerups[i].hitbox.width) / 2;
      float pw_y = model->powerups[i].hitbox.y - (pw_h - model->powerups[i].hitbox.height) / 2;
  ...
  if (model->saucer.alive) {
    float s_scale = 1.5f;
    SDL_FRect s_dst = {
        (float)model->saucer.hitbox.x - (model->saucer.hitbox.width * (s_scale - 1) / 2), 
        (float)model->saucer.hitbox.y - (model->saucer.hitbox.height * (s_scale - 1) / 2),
  ...
  if (model->boss.alive) {
    ...
    SDL_FRect boss = {(float)model->boss.hitbox.x - (model->boss.hitbox.width * (b_scale - 1) / 2), 
                      (float)model->boss.hitbox.y - (model->boss.hitbox.height * (b_scale - 1) / 2),
    ...
    for (int i = 0; i < INVADER_ROWS; i++) {
      for (int j = 0; j < INVADER_COLS; j++) {
        ...
        if (inv->alive) {
          float i_scale = 1.3f;
          SDL_FRect idst = {(float)inv->hitbox.x - (inv->hitbox.width * (i_scale - 1) / 2), 
...
void sdl_view_render(SDLView *view, const GameModel *model) {
  ...
  else if (model->state == STATE_PLAYING && (model->players[0].level == 4 || model->boss.alive) &&
  ...
  case STATE_WIN: {
    ...
    char buf[64];
    snprintf(buf, 64, "Final Score: %d", model->players[0].score + model->players[1].score);
