  ...
  for (int i = 0; i < 10; i++) {
    if (model->powerups[i].alive) {
      ...
      switch(model->powerups[i].type) {
          case PWR_TRIPLE_SHOT: symbol = "T"; break;
          case PWR_STRONG_MISSILE: symbol = "S"; break;
          case PWR_SHIELD: symbol = "#"; break;
          default: break;
      }
  ...
  for (int p=0; p<2; p++) {
    ...
    for (int i = 0; i < PLAYER_BULLETS; ++i) {
      if (model->player_bullets[p][i].alive) {
        int x = view->game_start_x + ncurses_scale_x(model->player_bullets[p][i].hitbox.x);
        int y = view->game_start_y + ncurses_scale_y(model->player_bullets[p][i].hitbox.y);
        mvaddch(y, x, '|');
      }
    }
    attroff(COLOR_PAIR(p == 0 ? 1 : 4));
...
void ncurses_view_render_menu(NcursesView *view, const GameModel *model) {
  ...
  case MENU_DIFFICULTY:
    ...
    for (int i = 0; i < 4; i++) {
      if (i == model->menu_selection) {
        ...
        mvprintw(cy + 6 + i * 3, cx - (int)strlen(diff_desc[i]) / 2, "%s", diff_desc[i]);
  ...
  case MENU_CONTROLS: {
    ...
    // Back button
    if (model->menu_selection == 10) attron(A_REVERSE);
    mvprintw(cy + 18, cx - 4, "> BACK <");
    if (model->menu_selection == 10) attroff(A_REVERSE);
    break;
  }
  }
