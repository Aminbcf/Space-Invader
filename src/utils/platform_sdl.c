...
uint32_t platform_get_ticks(void) { return (uint32_t)SDL_GetTicks(); }
...
bool platform_key_pressed(int key) {
  ...
  /* SDL3 SDL_GetScancodeFromKey requires keymod state pointer, passing NULL is
   * valid */
  SDL_Scancode scancode = SDL_GetScancodeFromKey((SDL_Keycode)key, NULL);

  /* Hack to support WASD even if controller asks for Arrows */
  if (key == SDLK_LEFT) {
    if ((int)SDL_SCANCODE_LEFT < num_keys && keyboard_state[SDL_SCANCODE_LEFT])
      return true;
    if ((int)SDL_SCANCODE_A < num_keys && keyboard_state[SDL_SCANCODE_A])
      return true;
  } else if (key == SDLK_RIGHT) {
    if ((int)SDL_SCANCODE_RIGHT < num_keys &&
        keyboard_state[SDL_SCANCODE_RIGHT])
      return true;
    if ((int)SDL_SCANCODE_D < num_keys && keyboard_state[SDL_SCANCODE_D])
      return true;
  } else if (key == SDLK_SPACE) {
    if ((int)SDL_SCANCODE_SPACE < num_keys &&
...
// STUBS for joystick to avoid linker errors if they are called (they should
// return false/0)
bool platform_joystick_available(void) { return false; }
float platform_joystick_axis(int joystick_id, int axis) {
  (void)joystick_id;
  (void)axis;
  return 0.0f;
