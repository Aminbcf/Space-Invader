...
void input_handler_process_sdl_event(InputHandler* handler, const SDL_Event* event) {
    ...
    switch (event->type) {
        case SDL_EVENT_KEY_DOWN:
            if (is_left)  handler->keyboard_state.left_pressed = true;
            if (is_right) handler->keyboard_state.right_pressed = true;
...
void input_handler_handle_joystick_event(InputHandler* handler, const SDL_Event* event) {
    ...
    if (joystick_id >= (SDL_JoystickID)handler->joystick_count) return;
    
    InputState* joy_state = &handler->joystick_states[joystick_id];
    
    switch (event->type) {
        case SDL_EVENT_JOYSTICK_BUTTON_DOWN:
            if (event->jbutton.button == handler->joy_button_left) joy_state->left_pressed = true;
            if (event->jbutton.button == handler->joy_button_right) joy_state->right_pressed = true;
            if (event->jbutton.button == handler->joy_button_shoot) joy_state->shoot_pressed = true;
            if (event->jbutton.button == handler->joy_button_pause) joy_state->pause_pressed = true;
            if (event->jbutton.button == handler->joy_button_quit) joy_state->quit_pressed = true;
            break;
...
void input_handler_process_ncurses_input(InputHandler* handler, int ch) {
    ...
    switch (ch) {
        case 'a': case 'A': handler->keyboard_state.left_pressed = true; break;
        case 'd': case 'D': handler->keyboard_state.right_pressed = true; break;
        case ' ': case '\n': handler->keyboard_state.shoot_pressed = true; break;
        case 'p': case 'P': handler->keyboard_state.pause_pressed = true; break;
        case 'q': case 'Q': case 27: handler->keyboard_state.quit_pressed = true; break;
    }
}
...
bool input_handler_get_command(InputHandler* handler, Command* cmd) {
    ...
    *cmd = CMD_NONE;
    ...
    if (state->quit_pressed) { *cmd = CMD_QUIT; state->quit_pressed = false; return true; }
    if (state->pause_pressed && !state->pause_was_pressed) { 
        *cmd = CMD_PAUSE; state->pause_was_pressed = true; state->pause_pressed = false; return true; 
    }
    if (state->left_pressed) { *cmd = CMD_MOVE_LEFT; return true; }
    if (state->right_pressed) { *cmd = CMD_MOVE_RIGHT; return true; }
    if (state->shoot_pressed && !state->shoot_was_pressed) { *cmd = CMD_SHOOT; return true; }
...
bool input_handler_is_key_pressed(InputHandler* handler, SDL_Keycode key) {
    ...
    if ((int)scancode >= num_keys) return false;
...
float input_handler_get_joystick_axis(InputHandler* handler, int joystick_id, int axis) {
    if (!handler || joystick_id < 0 || joystick_id >= handler->joystick_count) return 0.0f;
...
bool input_handler_get_joystick_button(InputHandler* handler, int joystick_id, int button) {
    if (!handler || joystick_id < 0 || joystick_id >= handler->joystick_count) return false;
