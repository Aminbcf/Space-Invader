...
Command controller_translate_input(Controller *controller, InputEvent *event) {
  ...
  case INPUT_KEYBOARD:
    // Player 1
    if (event->key == controller->key_up) return CMD_UP;
    if (event->key == controller->key_down) return CMD_DOWN;
    if (event->key == controller->key_left) return CMD_LEFT;
    if (event->key == controller->key_right) return CMD_RIGHT;
    if (event->key == controller->key_shoot) return CMD_SHOOT;
    if (event->key == controller->key_pause) return CMD_PAUSE;
    if (event->key == controller->key_quit) return CMD_QUIT;
    
    // Player 2
    if (event->key == controller->key_p2_up) return CMD_P2_MOVE_UP;
    if (event->key == controller->key_p2_down) return CMD_P2_MOVE_DOWN;
    if (event->key == controller->key_p2_left) return CMD_P2_MOVE_LEFT;
    if (event->key == controller->key_p2_right) return CMD_P2_MOVE_RIGHT;
    if (event->key == controller->key_p2_shoot) return CMD_P2_SHOOT;
    
    // Hardcoded shortcuts
    if (event->key == 'r' || event->key == 'R') return CMD_RESET_GAME;
    if (event->key == 13 || event->key == 10) return CMD_CONFIRM; // Enter
    if (event->key == 27) return CMD_BACK; // ESC
    break;
  ...
  case INPUT_JOYSTICK:
    ...
    if (event->axis == 0) {
      if (event->value < -16000) return CMD_LEFT;
      if (event->value > 16000) return CMD_RIGHT;
    }
    break;
  default: break;
  }
...
void controller_execute_command(Controller *controller, Command cmd) {
  ...
  if (controller->model->state == STATE_GAME_OVER || controller->model->state == STATE_WIN) {
      if (cmd == CMD_RESET_GAME) { model_reset_game(controller->model); controller->model->state = STATE_PLAYING; }
      if (cmd == CMD_BACK) { controller->model->state = STATE_MENU; }
      return;
  }
  ...
  switch (cmd) {
  // P1 Movement
  case CMD_LEFT: model_move_player(controller->model, 0, DIR_LEFT); break;
  case CMD_RIGHT: model_move_player(controller->model, 0, DIR_RIGHT); break;
  case CMD_UP: model_move_player(controller->model, 0, DIR_UP); break;
  case CMD_DOWN: model_move_player(controller->model, 0, DIR_DOWN); break;
  case CMD_SHOOT: model_player_shoot(controller->model, 0); break;

  // P2 Movement
  case CMD_P2_MOVE_LEFT: model_move_player(controller->model, 1, DIR_LEFT); break;
  case CMD_P2_MOVE_RIGHT: model_move_player(controller->model, 1, DIR_RIGHT); break;
  case CMD_P2_MOVE_UP: model_move_player(controller->model, 1, DIR_UP); break;
  case CMD_P2_MOVE_DOWN: model_move_player(controller->model, 1, DIR_DOWN); break;
  case CMD_P2_SHOOT: model_player_shoot(controller->model, 1); break;

  case CMD_PAUSE: model_toggle_pause(controller->model); break;
  case CMD_RESET_GAME: model_reset_game(controller->model); break;
  case CMD_BACK: model_set_state(controller->model, STATE_MENU); break;
  case CMD_QUIT: controller->quit_requested = true; break;
  
  default: break;
  }

  if (controller->render_callback) controller->render_callback(controller->callback_data);
}
